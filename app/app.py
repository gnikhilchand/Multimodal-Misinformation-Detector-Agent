...
import requests
import gradio as gr
from transformers import AutoTokenizer, AutoModelForSequenceClassification, pipeline, CLIPProcessor, CLIPImageProcessor, CLIPModel
...
image_model = "group9-dsailab/Nikhil"
#image_model = "group9-dsailab/finetuned_clip_patch16"
...
def get_image_classifier():
    ...
    filename = "best_clip_finetuned_classifier.pth"
...
def get_evidence_sentences(claim):
    # --- GOOGLE: take max 1 ---
    g = _safe_call(_from_google, claim)
    g = g[:1]

    # --- TAVILY: take max 2 ---
    t = _safe_call(_from_tavily, claim)
    t = t[:2]

    # --- WIKIPEDIA: take max 2 ---
    w = _safe_call(_from_wiki, claim)
    w = w[:2]

    # combine all
    evid = g + t + w

    # clean up empty/short lines
    evid = [e for e in evid if len(e.strip()) > 10]

    # safety fallback
    return evid or ["Error: No relevant evidence found."]
...
def classify_text(claim, hf_token, tavily_key, google_key):
    ...
    prompt = f"""
            ...
            Now, carefully evaluate the statement and the assessment. You must disagree with the system if the evidences clearly and heavily contradict the claim.
...
"""
    ...
    completion = inf_client.chat_completion( model=explain_model, messages=messages, max_tokens=256, temperature=0.4)
